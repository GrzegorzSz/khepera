 ...
 *        
 * \author   C�dric Gaudin (K-Team SA)                               
 *
...
/*--------------------------------------------------------------------*/
/*! 
 * This function reads a word (16-bit) value from a given K-Net device in one sequence
 *
 * \param dev   K-Net device descriptor
 * \param reg   K-Net register address
 * \param val   A pointer to a variable that will receive the 
 ...
 * \remark This function is thread safe.
 */
int knet_read_word( knet_dev_t * dev , 
		 knet_reg_t reg , 
		 unsigned short * val ) 
{
  int rc;
  ...
  if ( dev->order & KNET_ORDER_BIT_SWAP ) {
    ...
    rc = knet_lltransfer( dev , &treg[0] , 1 , (char *)&p[0] , 2 );
    if ( rc < 0 )
      return rc;


  /* need to convert result from big endian */
  if ( (dev->order & KNET_ORDER_BIG ) || (dev->order &KNET_ORDER_MSB_FIRST ))  {
    t = p[0];
    p[0] = p[1];
    p[1] = t;
  }
...
/*--------------------------------------------------------------------*/
/*! 
 * This function writes a Word (16 bit) value to a given K-Net device on one write sequence (will write on the same register addr).
 *
 * \param dev   K-Net device descriptor
 * \param reg   K-Net register address 
 * \param val   a 16-bit value
 ...
 * \remark This function is thread safe.
 */
int knet_write_word( knet_dev_t * dev ,
		  knet_reg_t reg ,
		  unsigned short val )
{
  unsigned char buf[4];
  ...
  if ( dev->order & KNET_ORDER_REP_ADR ) {
    ...
    if ( dev->order & KNET_ORDER_MSB_FIRST ) {
      ...
      if ((rc=knet_llwrite( dev , buf , 3 )) < 0 )
	return rc;
